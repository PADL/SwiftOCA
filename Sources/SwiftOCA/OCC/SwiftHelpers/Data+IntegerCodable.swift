...
extension Data {
    ...
    mutating func encodeInteger<T: FixedWidthInteger>(_ value: T, index: Int, endianness: Endianness = .big) {
        precondition(index >= 0)
        precondition(index <= self.count - MemoryLayout<T>.size)
        ...
        self.replaceSubrange(index..<lastIndex, with: withUnsafePointer(to: byteSwappedValue) { unbound -> Data in
            return unbound.withMemoryRebound(to: UInt8.self, capacity: MemoryLayout<T>.size) { bytes -> Data in
