...
public actor OcaConnectionBroker {
  ...
  struct DeviceInfo: Sendable, Hashable {
    ...
    var host: String? { netService.hostName! }
    var port: UInt16 { UInt16(netService.port) }

    static func == (lhs: Self, rhs: Self) -> Bool {
      lhs.deviceIdentifier == rhs.deviceIdentifier &&
        lhs.netService.addresses == rhs.netService.addresses
    }

    func hash(into hasher: inout Hasher) {
      deviceIdentifier.hash(into: &hasher)
      netService.addresses?.hash(into: &hasher)
    }
  }
  ...
  private final class BrowserMonitor {
    let browser: OcaBrowser
    var browserMonitor: Task<(), Error>?

    init(serviceType: OcaNetworkAdvertisingServiceType, broker: OcaConnectionBroker) {
      browser = OcaBrowser(serviceType: serviceType)
      browserMonitor = Task { [weak broker] in
        for try await result in browser.channel {
  ...
  private func _onBrowseResult(_ result: OcaBrowser.Result) async throws {
    var expiringConnection: DeviceConnection?

    let event = try await Event(result: result) // will resolve
    let netService = try! result.result.get() // at this point we know it is valid
    let device = DeviceInfo(deviceIdentifier: event.deviceIdentifier, netService: netService)
    ...
    case .deviceAdded:
         ...
         let existingConnection = _removeConnection(for: event.deviceIdentifier)
      {
        ...
        if let mutableConnection = existingConnection.connection as? Ocp1MutableConnection,
           let deviceAddress = device.resolveFirst()
        {
          mutableConnection.deviceAddress = deviceAddress
        }
      }
  ...
  public init(connectionOptions: Ocp1ConnectionOptions = .init()) {
    ...
    {
      browsers[serviceType] = BrowserMonitor(serviceType: serviceType, broker: self)
    }
    _browsers = browsers
  }
  ...
  func _open(
    ...
    guard let address = deviceInfo.resolveFirst() else {
...
extension OcaConnectionBroker.DeviceIdentifier {
  init(netService: NetService) async throws {
    ...
    try await withCheckedThrowingContinuation { continuation in
      ...
      netService.delegate = resolverHelper
      netService.schedule(in: RunLoop.main, forMode: .default)
      netService.resolve(withTimeout: 1)
    }

    netService.remove(from: RunLoop.main, forMode: .default)
    netService.delegate = nil

    guard let txtRecordData = netService.txtRecordData()
    else { throw Ocp1Error.remoteDeviceResolutionFailed }
    let txtRecord: [String: String] = NetService.dictionary(fromTXTRecord: txtRecordData)

    guard let modelGUID = txtRecord["modelGUID"],
          let serialNumber = txtRecord["serialNumber"],
          let serviceType = OcaNetworkAdvertisingServiceType(rawValue: netService.type)
    else {
      throw Ocp1Error.remoteDeviceResolutionFailed
    }

    try self.init(
      serviceType: serviceType, modelGUID: OcaModelGUID(modelGUID),
...
extension OcaConnectionBroker.DeviceInfo {
  func resolve() -> [Data]? {
    netService.addresses
  }

  func resolveFirst() -> Data? {
    resolve()?.first
  }
}

extension OcaConnectionBroker.Event {
  init(result: OcaBrowser.Result) async throws {
    let netService = try result.result.get()

    let deviceIdentifier = try await OcaConnectionBroker.DeviceIdentifier(netService: netService)
