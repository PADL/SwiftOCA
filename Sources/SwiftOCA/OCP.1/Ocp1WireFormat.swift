...
/// On a message-oriented transport one read must return exactly one complete PDU;
/// packet boundaries are never crossed to complete a malformed packet.
///
...
package final class Ocp1PduReader: OcaPduReader {
  ...
  package func nextPdu(
    ...
    guard preservesPduBoundaries else {
    ...
    try Self.validatePacket(messagePduData, maximumPduSize: maximumPduSize)
  ...
  private func nextStreamPdu(
    ...
    let length = try Self.pduLength(buffer, at: offset, maximumPduSize: maximumPduSize)
  ...
  package static func validatePacket(_ data: Data, maximumPduSize: Int) throws {
    guard data.count >= OcaConnection.MinimumPduSize else {
      throw Ocp1Error.pduTooShort
    }
    guard try pduLength(data, at: 0, maximumPduSize: maximumPduSize) == data.count else {
      throw Ocp1Error.invalidPduSize
    }
  }

  private static func pduLength(
    _ data: Data,
    at offset: Int,
    maximumPduSize: Int
  ) throws -> Int {
