...
### Manifest gating: use a trait, not `#if os(Android)`
...
manifest is never true. FlutterSwift works around this with a `FLUTTER_SWIFT_JVM`
environment variable. SwiftOCA already has a traits vocabulary
(`Package.swift:347`, `NonEmbeddedBuild`), so a trait — say `AndroidNSD` — is the
idiomatic fit and keeps swift-java out of the dependency graph for everyone else.
...
2. Stand up the `SwiftOCAAndroid` target, trait, and `swift-java.config` with a
